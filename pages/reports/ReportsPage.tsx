        ...
        try {
            switch (reportType) {
                case 'sales_summary': {
                    ...
                    setReportContent(
                        <><h3 className="font-semibold mb-4">ข้อมูลยอดขายระหว่างวันที่ {startDate} ถึง {endDate}</h3><DataTable columns={salesColumns} data={data} /><p className="text-right font-bold text-lg mt-4">ยอดรวมทั้งหมด: {new Intl.NumberFormat('th-TH', { minimumFractionDigits: 2 }).format(total)} บาท</p></>
                    );
                ...
                case 'sales_by_customer': {
                 ...
                 case 'top_profit_products': {
                    ...
                    const reportData: ProductProfitReportItem[] = Object.values(productProfits).sort((a: ProductProfitReportItem, b: ProductProfitReportItem) => (b.profit || 0) - (a.profit || 0));
                    const columns: Column<ProductProfitReportItem>[] = [
                         ...
                         { header: 'ยอดขาย', accessor: item => new Intl.NumberFormat('th-TH', {minimumFractionDigits: 2}).format(item.revenue ?? 0), className: 'text-right'},
                         { header: 'ต้นทุน', accessor: item => new Intl.NumberFormat('th-TH', {minimumFractionDigits: 2}).format(item.cost ?? 0), className: 'text-right'},
                         { header: 'กำไร', accessor: item => <span className="font-bold text-green-600">{new Intl.NumberFormat('th-TH', {minimumFractionDigits: 2}).format(item.profit ?? 0)}</span>, className: 'text-right'},
                    ];
                ...
                case 'plot_production': {
                    ...
                    if (error) throw error;
                    const reportData = (data as Plot[] || []).map(plot => {
