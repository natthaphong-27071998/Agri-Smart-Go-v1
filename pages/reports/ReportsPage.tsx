...
import { SalesOrder, FinancialTransaction, Employee, Plot, Product } from '../../types';
        ...
        try {
            switch (reportType) {
                case 'sales_summary': {
                    ...
                    const total = data.reduce((sum, o) => sum + (o.totalAmount || 0), 0);
                    setReportContent(
                        <><h3 className="font-semibold mb-4">ข้อมูลยอดขายระหว่างวันที่ {startDate} ถึง {endDate}</h3><DataTable columns={salesColumns} data={data} /><p className="text-right font-bold text-lg mt-4">ยอดรวมทั้งหมด: {total.toLocaleString('th-TH', { minimumFractionDigits: 2 })} บาท</p></>
                    );
                    setReportDataForExport(data); setReportColumnsForExport(salesColumns); setIsExportable(true);
                    break;
                }
                case 'sales_by_customer': {
                    ...
                    const salesByCustomer = data.reduce((acc, order) => {
                        if (order.customerName) {
                           acc[order.customerName] = (acc[order.customerName] || 0) + (order.totalAmount || 0);
                        }
                        return acc;
                    ...
                    const columns: Column<typeof reportData[0]>[] = [
                        ...
                        { header: 'ยอดขายรวม (บาท)', accessor: item => item.total.toLocaleString('th-TH', { minimumFractionDigits: 2 }), className: 'text-right' },
                    ];
                 ...
                 case 'top_profit_products': {
                    const { data: orders, error: orderError } = await supabase.from('sales_orders').select('*, items:order_items(*)').gte('orderDate', startDate).lte('orderDate', endDate).neq('status', 'Cancelled');
                    const { data: products, error: productError } = await supabase.from('products').select('id, costPrice');
                    ...
                    const reportData: ProductProfitReportItem[] = Object.values(productProfits).sort((a: ProductProfitReportItem, b: ProductProfitReportItem) => b.profit - a.profit);
                    const columns: Column<ProductProfitReportItem>[] = [
                         { header: 'สินค้า', accessor: 'name'}, { header: 'จำนวนที่ขาย', accessor: item => (item.quantity ?? 0).toLocaleString(), className: 'text-right'},
                         { header: 'ยอดขาย', accessor: item => (item.revenue ?? 0).toLocaleString('th-TH', {minimumFractionDigits: 2}), className: 'text-right'},
                         { header: 'ต้นทุน', accessor: item => (item.cost ?? 0).toLocaleString('th-TH', {minimumFractionDigits: 2}), className: 'text-right'},
                         { header: 'กำไร', accessor: item => <span className="font-bold text-green-600">{(item.profit ?? 0).toLocaleString('th-TH', {minimumFractionDigits: 2})}</span>, className: 'text-right'},
                    ];
                ...
                case 'plot_production': {
                    ...
                    if (error) throw error;
                    const reportData = (data || []).map(plot => {
                        const plotSize = plot.size || 0;
                        const cost = (plotSize * 2500) + (plot.status === 'กำลังเติบโต' ? 1200 : 0); // Mock cost
                        const yieldVal = plot.status === 'พร้อมเก็บเกี่ยว' ? (plotSize * 500) : (plot.status === 'กำลังเติบโต' ? plotSize * 150 : 0); // Mock yield
                        return { id: plot.id, name: plot.name, crop: plot.crop, cost, yield: yieldVal, yield_per_rai: plotSize > 0 ? yieldVal / plotSize : 0 };
                    });
                    const columns: Column<typeof reportData[0]>[] = [
                        { header: 'แปลง', accessor: 'name' }, { header: 'พืช', accessor: 'crop' },
                        { header: 'ต้นทุนประมาณการ (บาท)', accessor: item => item.cost.toLocaleString(), className: 'text-right' },
                        { header: 'ผลผลิตประมาณการ (กก.)', accessor: item => item.yield.toLocaleString(), className: 'text-right' },
                ...
                case 'pnl_statement': {
                    ...
                    if (error) throw error;
                    const totalIncome = (data || []).filter(t => t.type === 'Income').reduce((sum, t) => sum + (t.amount || 0), 0);
                    const totalExpense = (data || []).filter(t => t.type === 'Expense').reduce((sum, t) => sum + (t.amount || 0), 0);
                    const netProfit = totalIncome - totalExpense;
                    setReportContent(
                        <div className="border border-gray-200 p-6 rounded-lg max-w-2xl mx-auto"><div className="mt-6 flow-root"><dl className="-my-4 divide-y divide-gray-200 text-base"><div className="flex items-center justify-between py-4"><dt className="font-medium text-green-600">รายได้รวม</dt><dd className="font-medium text-green-600">฿{totalIncome.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</dd></div><div className="flex items-center justify-between py-4"><dt className="font-medium text-red-600">ค่าใช้จ่ายรวม</dt><dd className="font-medium text-red-600">- ฿{totalExpense.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</dd></div><div className="flex items-center justify-between py-5 border-t-2 border-gray-300 mt-4"><dt className="font-bold text-gray-900">กำไร/ขาดทุน สุทธิ</dt><dd className={`font-bold ${netProfit >= 0 ? 'text-gray-900' : 'text-red-700'}`}>฿{netProfit.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</dd></div></dl></div></div>
                    );
                    break;
                }
                case 'balance_sheet': {
                ...
                case 'active_employees': {
                    ...
                    if(error) throw error;
                    const employeeColumns: Column<Omit<Employee, 'status'>>[] = [
                ...
                case 'payroll_summary': {
                    ...
                    const totalSalary = (data || []).reduce((sum, emp) => sum + (emp.salary || 0), 0);
                     setReportContent(<div className="text-center p-8"><p className="text-lg text-gray-600">ยอดรวมเงินเดือนพนักงานที่ใช้งานอยู่</p><p className="text-4xl font-bold text-blue-600 mt-2">฿{totalSalary.toLocaleString('th-TH', {minimumFractionDigits: 2})}</p><p className="text-sm text-gray-500 mt-1">จากพนักงานทั้งหมด {(data || []).length} คน</p></div>);
                    setIsExportable(false);
                    break;
                }
                default:
