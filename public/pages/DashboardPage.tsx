...
const QuickActionButton: React.FC<{ icon: React.ReactNode; title: string; to: string; color: string; darkColor: string; }> = ({ icon, title, to, color, darkColor }) => (
    <Link to={to} className={`flex items-center p-4 bg-white dark:bg-gray-800 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors shadow-sm border dark:border-gray-700`}>
...
const DashboardPage: React.FC = () => {
    const { data, loading } = useSupabase<DashboardData>(async () => {
        ...
        const [
            ...
            supabase.from('financial_transactions').select('*').gte('date', sixMonthsAgo),
            ...
            supabase.from('farm_activities').select('*').order('date', { ascending: false }).limit(5)
        ]);
    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Link to="/accounting"><StatCard title="รายรับวันนี้" value={`฿${stats.todayIncome.toLocaleString()}`} color="bg-green-100 text-green-600" darkColor="dark:bg-green-900/50 dark:text-green-300" icon={ICONS.ACCOUNTING}/></Link>
                <Link to="/sales"><StatCard title="ยอดขายเดือนนี้" value={`฿${stats.monthSales.toLocaleString()}`} color="bg-blue-100 text-blue-600" darkColor="dark:bg-blue-900/50 dark:text-blue-300" icon={ICONS.SALES}/></Link>
                <Link to="/sales"><StatCard title="ออเดอร์ใหม่" value={`${stats.newOrders} รายการ`} color="bg-red-100 text-red-600" darkColor="dark:bg-red-900/50 dark:text-red-300" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>} /></Link>
                <Link to="/hr"><StatCard title="พนักงานที่ทำงานอยู่" value={`${stats.activeEmployees} คน`} color="bg-yellow-100 text-yellow-600" darkColor="dark:bg-yellow-900/50 dark:text-yellow-300" icon={ICONS.HR}/></Link>
                <Link to="/production"><StatCard title="แปลงพร้อมเก็บเกี่ยว" value={`${stats.harvestReadyPlots} แปลง`} color="bg-teal-100 text-teal-600" darkColor="dark:bg-teal-900/50 dark:text-teal-300" icon={ICONS.PRODUCTION}/></Link>
                <Link to="/investment"><StatCard title="กำลังลงทุน" value={`฿${stats.ongoingInvestment.toLocaleString()}`} color="bg-purple-100 text-purple-600" darkColor="dark:bg-purple-900/50 dark:text-purple-300" icon={ICONS.INVESTMENT}/></Link>
                <Link to="/investment"><StatCard title="ความเสี่ยงสูง" value={`${stats.highRisks} รายการ`} color="bg-orange-100 text-orange-600" darkColor="dark:bg-orange-900/50 dark:text-orange-300" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>}/></Link>
                <Link to="/production"><StatCard title="อุปกรณ์ออนไลน์" value={`${stats.onlineDevices} เครื่อง`} color="bg-indigo-100 text-indigo-600" darkColor="dark:bg-indigo-900/50 dark:text-indigo-300" icon={ICONS.WIFI}/></Link>
            </div>
                    ...
                    <div className="space-y-3">
                        {recentActivities.length > 0 ? recentActivities.map(activity => (
                         ...
                         <div className="text-center pt-2">
                             <Link to="/production" className="text-sm font-semibold text-green-600 hover:text-green-500">ดูกิจกรรมทั้งหมด &rarr;</Link>
                        </div>
                    </div>
                </div>
            </div>
