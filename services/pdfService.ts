...
const generatePdfBase = (
    ...
    doc.save(`${docType.split(' ')[0].toLowerCase()}_${order.id}.pdf`);
};
...
export const generateOrderPdf = (order: SalesOrder, company: CompanyInfo) => {
    generatePdfBase(order, company, 'Purchase Order', (doc) => {
        ...
        doc.setFontSize(11);
        doc.text(`รหัสสั่งซื้อ: ${order.id}`, 40, 180);
...
export const generateInvoicePdf = (order: SalesOrder, company: CompanyInfo) => {
     generatePdfBase(order, company, 'Invoice', (doc) => {
        ...
        doc.text(`เลขที่ใบแจ้งหนี้: INV-${order.id}`, 40, 180);
        doc.text(`อ้างอิงใบสั่งซื้อ: ${order.id}`, 40, 195);
        ...
        const tableColumn = ["รหัสสินค้า", "ชื่อสินค้า", "จำนวน", "ราคา/หน่วย", "รวม"];
        const tableRows: any[] = order.items.map(item => [
            item.productId, item.productName, item.quantity, (item.unitPrice ?? 0).toFixed(2), (item.total ?? 0).toFixed(2)
        ]);
        (doc as any).autoTable({
        ...
        if (company.bankAccount) {
            ...
            doc.setFontSize(10);
            doc.text(`ธนาคาร: ${company.bankAccount.bankName}`, 40, finalY + 105);
            doc.text(`ชื่อบัญชี: ${company.bankAccount.accountName}`, 40, finalY + 120);
            doc.text(`เลขที่บัญชี: ${company.bankAccount.accountNumber}`, 40, finalY + 135);
        }
    });
};
