...
const createItemsTable = (order: SalesOrder) => {
    ...
    const header = new docx.TableRow({
        tableHeader: true,
        children: [
            tableCell("รหัสสินค้า", { run: { bold: true } }),
    ...
    const rows = order.items.map(item => new docx.TableRow({
        children: [
            tableCell(item.product_code ?? 'N/A'),
...
export const generateOrderDoc = async (order: SalesOrder, company: CompanyInfo) => {
    ...
    const doc = new docx.Document({
        ...
        sections: [{
            children: [
                ...
                new docx.Paragraph({ text: `รหัสสั่งซื้อ: ${order.order_code}` }),
    ...
    docx.Packer.toBlob(doc).then((blob: any) => {
        saveAs(blob, `purchase_order_${order.order_code}.docx`);
    });
};
...
export const generateInvoiceDoc = async (order: SalesOrder, company: CompanyInfo) => {
    ...
    const doc = new docx.Document({
        ...
        sections: [{
            children: [
                ...
                new docx.Paragraph({ text: `เลขที่ใบแจ้งหนี้: INV-${order.order_code}` }),
                new docx.Paragraph({ text: `อ้างอิงใบสั่งซื้อ: ${order.order_code}` }),
                ...
                ...(company.bankAccount && typeof company.bankAccount === 'object' && 'bankName' in company.bankAccount ? (() => {
                    ...
                    return [
                        ...
                        new docx.Paragraph({ text: `ธนาคาร: ${bankAccount.bankName}` }),
                        new docx.Paragraph({ text: `ชื่อบัญชี: ${bankAccount.accountName}` }),
                        new docx.Paragraph({ text: `เลขที่บัญชี: ${bankAccount.accountNumber}` }),
                    ]
                })() : [])
            ],
        }],
    ...
    docx.Packer.toBlob(doc).then((blob: any) => {
        saveAs(blob, `invoice_${order.order_code}.docx`);
    });
};