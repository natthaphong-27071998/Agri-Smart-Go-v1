    ...
    useEffect(() => {
     ...
     useEffect(() => {
        if (isOpen && !chat) {
             ...
             if (!apiKey) {
                 console.error("Gemini API key is not configured. Please set API_KEY in your environment.");
                 setMessages(prev => [...prev, { role: 'model', parts: 'ขออภัยค่ะ ไม่ได้ตั้งค่า API Key สำหรับผู้ช่วย AI' }]);
                 return;
             }
    ...
    return (
        <>
            ...
            <div className={`fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-end sm:items-center transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                 <div className={`bg-white dark:bg-gray-800 rounded-t-lg sm:rounded-lg shadow-xl w-full max-w-2xl h-[90vh] sm:h-[80vh] flex flex-col m-0 sm:m-4 transform transition-transform duration-300 ${isOpen ? 'translate-y-0' : 'translate-y-full sm:translate-y-10'}`} onClick={(e) => e.stopPropagation()}>
                        ...
                        {messages.map((msg, index) => (
                            <div key={index} className={`flex items-end gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                ...
                                <div className={`max-w-xs md:max-w-md lg:max-w-lg px-4 py-2 rounded-2xl shadow-sm ${msg.role === 'user' ? 'bg-green-600 text-white rounded-br-none' : 'bg-gray-100 text-gray-800 rounded-bl-none'}`}>
                                    {msg.role === 'model' ? (
                                        <ReactMarkdown remarkPlugins={[remarkGfm]} className="prose prose-sm max-w-none prose-p:my-2 prose-headings:my-3 dark:prose-invert">
                                            {msg.parts}
                                        </ReactMarkdown>
                                    ) : (
